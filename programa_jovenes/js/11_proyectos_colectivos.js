        ...
        function renderSolicitudesHTML(proyecto, jovenId) {
            ...
            return `<div class="mt-3 pt-3 border-t border-orange-200 bg-orange-50 rounded-lg p-3">
                ...
                ${sols.map((s, idx) => `<div class="flex items-center gap-2 bg-white border border-orange-200 rounded-lg p-2 mb-1.5 shadow-sm">
                    ...
                    <button onclick="aprobarSolicitud(${jovenId}, ${proyecto.id}, '${escapeHtml(s.run)}')" class="bg-green-500 hover:bg-green-600 text-white border-none rounded-md px-2 py-1 text-[0.65rem] font-bold cursor-pointer transition" title="Aprobar"><i class="fas fa-check"></i></button>
                    <button onclick="rechazarSolicitud(${jovenId}, ${proyecto.id}, '${escapeHtml(s.run)}')" class="bg-red-400 hover:bg-red-500 text-white border-none rounded-md px-2 py-1 text-[0.65rem] font-bold cursor-pointer transition" title="Rechazar"><i class="fas fa-times"></i></button>
                </div>`).join('')}
            </div>`;
        }

        async function aprobarSolicitud(jovenId, proyectoId, solicitanteRun) {
            const joven = personasJovenes.find(j => j.id === jovenId);
            if (!joven) return;
            const proyecto = joven.camino?.proyectos_colectivos?.find(p => p.id === proyectoId);
            if (!proyecto) return;
            const solIdx = (proyecto.solicitudes_pendientes || []).findIndex(s => s.run === solicitanteRun);
            if (solIdx === -1) return;
            const solicitud = proyecto.solicitudes_pendientes[solIdx];

            try {
                mostrarNotificacion('info', 'Aprobando solicitud de ' + solicitud.nombre + '...');
                if (!proyecto.participantes) proyecto.participantes = [];
                proyecto.participantes.push({ run: solicitud.run, nombre: solicitud.nombre, tipo: 'joven', foto: solicitud.foto });
                proyecto.solicitudes_pendientes.splice(solIdx, 1);

                await sincronizarProyectoConParticipantes(proyecto, jovenId, false);
                
                // Enviar notificación por email
                await enviarNotificacionProyecto(solicitud.run, solicitud.nombre, proyecto, 'Participante', joven.nombre);
                
                mostrarNotificacion('exito', solicitud.nombre + ' fue aprobado e incorporado al proyecto.');
        ...
        async function rechazarSolicitud(jovenId, proyectoId, solicitanteRun) {
            const joven = personasJovenes.find(j => j.id === jovenId);
            if (!joven) return;
            const proyecto = joven.camino?.proyectos_colectivos?.find(p => p.id === proyectoId);
            if (!proyecto) return;
            const solIdx = (proyecto.solicitudes_pendientes || []).findIndex(s => s.run === solicitanteRun);
            if (solIdx === -1) return;
            const nombre = proyecto.solicitudes_pendientes[solIdx].nombre;

            try {
                proyecto.solicitudes_pendientes.splice(solIdx, 1);
                await sincronizarProyectoConParticipantes(proyecto, jovenId, false);
                mostrarNotificacion('info', 'Solicitud de ' + nombre + ' rechazada.');
                renderYouthProfile(joven);
            } catch (err) { console.error(err); mostrarNotificacion('error', 'Error: ' + err.message); }
        }
