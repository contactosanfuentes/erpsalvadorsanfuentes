...
const ETAPAS_PIONERO=[
  {n:'Cruz del Sur', img:'https://i.imgur.com/IStoscc.png', d:'Me integro a la Avanzada y descubro la vida de equipo'},
  {n:'Sendero',      img:'https://i.imgur.com/VHZrlFN.png', d:'Asumo responsabilidades y aporto en las Empresas'},
  {n:'Cumbre',       img:'https://i.imgur.com/3MeclHS.png', d:'Lidero desafíos y acompaño a mi equipo a la cima'}
];
function renderProgresionPionera(){
  ...
  c.innerHTML=ETAPAS_PIONERO.map((e,i)=>{
    ...
    return `<div style="display:flex;flex-direction:column;align-items:center;flex:1;opacity:${estado===''&&idx>=0?'0.45':'1'};" title="${e.d}">
      <img src="${e.img}" style="width:44px;height:44px;object-fit:contain;${estado==='actual'?'filter:drop-shadow(0 0 6px #f97316);':''}" onerror="this.style.display='none'">
      <span style="font-size:10px;font-weight:800;color:${estado==='actual'?'#C2410C':estado==='lograda'?'#16a34a':'#6b7280'};margin-top:4px;text-align:center;">${e.n}</span>
      ${estado==='actual'?'<span style="font-size:8px;font-weight:800;color:#C2410C;text-transform:uppercase;">● Etapa actual</span>':estado==='lograda'?'<span style="font-size:8px;color:#16a34a;font-weight:800;">✓ Lograda</span>':''}
    </div>`;
  }).join('<i class="fas fa-chevron-right" style="color:#fdba74;font-size:10px;"></i>');
}
// ══════════ MANIFIESTO ══════════
